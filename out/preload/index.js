...
const kilnApi = {
  fs: {
    ...
    watch: (dirPath, cb) => {
      ...
      electron.ipcRenderer.send("fs:watchStart", dirPath, watchId);
      return () => {
        electron.ipcRenderer.removeListener(channel, listener);
        electron.ipcRenderer.send("fs:watchStop");
      };
    }
  },
  git: {
    status: (root) => electron.ipcRenderer.invoke("git:status", root),
    stage: (root, filePath) => electron.ipcRenderer.invoke("git:stage", root, filePath),
    unstage: (root, filePath) => electron.ipcRenderer.invoke("git:unstage", root, filePath),
    commit: (root, message) => electron.ipcRenderer.invoke("git:commit", root, message),
    diff: (root, filePath) => electron.ipcRenderer.invoke("git:diff", root, filePath)
  },
  ollama: {
  ...
  terminal: {
    ...
    dispose: (id) => electron.ipcRenderer.send("terminal:dispose", id)
  },
  agent: {
    run: (req) => electron.ipcRenderer.invoke("agent:run", req),
    respondApproval: (callId, approved) => electron.ipcRenderer.invoke("agent:respondApproval", callId, approved),
    onEvent: (cb) => {
...
electron.contextBridge.exposeInMainWorld("kiln", kilnApi);
