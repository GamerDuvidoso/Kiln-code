...
const kilnApi: KilnApi = {
  fs: {
    ...
    watch: (dirPath, cb) => {
      ...
      const listener = (_e: unknown, changedPath: string) => cb(changedPath)
      ipcRenderer.on(channel, listener)
      ipcRenderer.send('fs:watchStart', dirPath, watchId)
      return () => {
        ipcRenderer.removeListener(channel, listener)
        ipcRenderer.send('fs:watchStop')
      }
    }
  },
  git: {
  ...
  agent: {
    run: (req: AgentRunRequest) => ipcRenderer.invoke('agent:run', req),
    respondApproval: (callId, approved) =>
      ipcRenderer.invoke('agent:respondApproval', callId, approved),
...
contextBridge.exposeInMainWorld('kiln', kilnApi)
